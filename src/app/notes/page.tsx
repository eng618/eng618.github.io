    ...
    <div className="w-full pb-8">
      <section className="py-12 text-center lg:py-16">
        <div className="container mx-auto px-4 lg:px-8">
          <h1 className="font-outfit mb-4 text-4xl font-bold lg:text-5xl">Notes</h1>
      ...
      <section className="py-8">
        <div className="container mx-auto px-4 lg:px-8">
          {notes.length === 0 ? (
            <p className="text-muted-foreground text-center italic">No notes found yet.</p>
          ) : (
                ...
                <Link key={note.slug} href={`/notes/${note.slug}`} className="group block h-full">
                  <Card className="border-border/50 bg-card/50 flex h-full flex-col backdrop-blur-sm transition-shadow hover:shadow-md">
                    <CardHeader className="flex-grow">
                      <div className="bg-primary/20 text-primary group-hover:bg-primary/30 mb-4 flex h-10 w-10 items-center justify-center rounded-lg transition-colors">
                        <FileText className="h-5 w-5" />
                      </div>
                      <CardTitle className="font-outfit text-xl font-bold">{note.title || note.slug}</CardTitle>
                      {note.description && (
